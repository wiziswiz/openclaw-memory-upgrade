...
// Pass 3: heuristic patterns for untagged bullets
const HEURISTIC_PATTERNS = [
  { pattern: /\b(prefers?|likes?|loves?|hates?|dislikes?|favorite|switched to)\b/i, type: 'preference' },
  { pattern: /\b(decided|chose|going with|committed to|plan to|will use)\b/i, type: 'decision' },
  { pattern: /\b(bought|sold|started|created|lives?|works?|moved|diagnosed|weighs|takes \d+mg)\b/i, type: 'fact' },
  { pattern: /\b(dose|mg|prescription|supplement|medication|blood|lab)\b/i, type: 'fact' },
];

const HEURISTIC_CONFIDENCE_DEFAULT = 0.7;

function classifyByHeuristic(content, config) {
  if (config && config.heuristicClassification === false) return null;
  if (content.length < 20) return null;
  for (const { pattern, type } of HEURISTIC_PATTERNS) {
    if (pattern.test(content)) {
      const confidence = (config && config.heuristicConfidence) || HEURISTIC_CONFIDENCE_DEFAULT;
...
function extractFacts(text, filePath, retainConfig) {
  ...
  // Pass 3: heuristic classification for untagged bullets
  for (let i = 0; i < lines.length; i++) {
    if (capturedLines.has(i + 1)) continue;
    const bulletMatch = lines[i].match(/^\s*[-*]\s+(.+)/);
    if (!bulletMatch) continue;
    const content = bulletMatch[1].trim();
    const classification = classifyByHeuristic(content, retainConfig);
    if (classification) {
      facts.push({
        content,
...
module.exports = { extractFacts, TAG_CONFIDENCE, TAG_TYPE, HEURISTIC_PATTERNS, classifyByHeuristic };
