...
CREATE TABLE IF NOT EXISTS dedup_reviews (
  id INTEGER PRIMARY KEY,
  new_chunk_id INTEGER,
  existing_chunk_id INTEGER NOT NULL,
...
function openDb(workspace) {
  ...
  try { db.exec('ALTER TABLE chunks ADD COLUMN file_weight REAL DEFAULT 1.0'); } catch (_) { /* migration: column may already exist */ }
  try { db.exec('ALTER TABLE chunks ADD COLUMN access_count INTEGER DEFAULT 0'); } catch (_) { /* migration: column may already exist */ }
  try { db.exec('ALTER TABLE chunks ADD COLUMN last_accessed TEXT'); } catch (_) { /* migration: column may already exist */ }
  try { db.exec('ALTER TABLE chunks ADD COLUMN stale INTEGER DEFAULT 0'); } catch (_) { /* migration: column may already exist */ }
  try { db.exec('ALTER TABLE chunks ADD COLUMN value_score REAL'); } catch (_) { /* migration: column may already exist */ }
  try { db.exec('ALTER TABLE chunks ADD COLUMN value_label TEXT'); } catch (_) { /* migration: column may already exist */ }
  try { db.exec('ALTER TABLE chunks ADD COLUMN content_updated_at TEXT'); } catch (_) { /* migration: column may already exist */ }
  try { db.exec('ALTER TABLE chunks ADD COLUMN source_type TEXT DEFAULT \'indexed\''); } catch (_) { /* migration: column may already exist */ }
  try { db.exec('ALTER TABLE chunks ADD COLUMN domain TEXT DEFAULT \'general\''); } catch (_) { /* migration: column may already exist */ }
  ...
  try {
    ...
    END;`);
  } catch (_) { /* migration: trigger may already exist */ }
  return db;
}
...
function insertChunks(db, filePath, mtimeMs, chunks, createdAt, dedupConfig, sourceType = 'indexed') {
  const now = new Date().toISOString();
  const weight = getFileWeight(filePath);
  const contentUpdatedAt = new Date(mtimeMs).toISOString();

  // Dedup pass: filter out duplicates if enabled
  let chunksToInsert = chunks;
  let dedupSkipped = 0;
  let dedupReviews = [];
  if (dedupConfig && dedupConfig.enabled) {
    const { dedupChunks } = require('./dedup');
    const result = dedupChunks(db, chunks, dedupConfig);
    chunksToInsert = result.toInsert;
    dedupSkipped = result.toSkip.length;
    dedupReviews = result.toReview;
  }

  const insert = db.prepare(`
    INSERT INTO chunks (file_path, heading, content, line_start, line_end, entities, chunk_type, confidence, created_at, indexed_at, file_weight, content_updated_at, source_type, domain)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const tx = db.transaction(() => {
    ...
    for (const c of chunksToInsert) {
      ...
      insert.run(filePath, c.heading || null, c.content, c.lineStart, c.lineEnd, JSON.stringify(c.entities), chunkType, confidence, createdAt || now, now, weight, contentUpdatedAt, sourceType, c.domain || 'general');
    }
    db.prepare(`INSERT OR REPLACE INTO files (file_path, mtime_ms, chunk_count, indexed_at) VALUES (?, ?, ?, ?)`)
      .run(filePath, mtimeMs, chunksToInsert.length, now);

    // Queue dedup reviews
    if (dedupReviews.length > 0) {
      const insertReview = db.prepare(`INSERT INTO dedup_reviews (new_chunk_id, existing_chunk_id, similarity, status, created_at) VALUES (?, ?, ?, 'pending', ?)`);
      for (const r of dedupReviews) {
        // Find the newly inserted chunk ID by matching content
        const newRow = db.prepare('SELECT id FROM chunks WHERE file_path = ? AND content = ? ORDER BY id DESC LIMIT 1').get(filePath, r.chunk.content);
        insertReview.run(newRow ? newRow.id : null, r.existingId, r.similarity, now);
      }
    }
  });
  tx();

  return { inserted: chunksToInsert.length, dedupSkipped, dedupReviews: dedupReviews.length };
}

function search(db, query, { limit = 10, sinceDate = null, untilDate = null, chunkType = null, minConfidence = null, includeStale = false, skipTracking = false, sourceType = null, domain = null } = {}) {
  let sql = `
...
function getDetailedStats(db) {
  ...
  const lowConfCount = db.prepare('SELECT COUNT(*) as n FROM chunks WHERE confidence < 0.3 AND stale = 0').get().n;
  let valueDist = [], sourceTypeDist = [], domainDist = [], chunkTypeDist = [];
  try { valueDist = db.prepare('SELECT value_label, COUNT(*) as count FROM chunks WHERE value_label IS NOT NULL GROUP BY value_label').all(); } catch (_) {}
  try { sourceTypeDist = db.prepare('SELECT source_type, COUNT(*) as count FROM chunks GROUP BY source_type').all(); } catch (_) {}
  try { domainDist = db.prepare('SELECT domain, COUNT(*) as count FROM chunks GROUP BY domain ORDER BY count DESC').all(); } catch (_) {}
  chunkTypeDist = db.prepare('SELECT chunk_type, COUNT(*) as count FROM chunks GROUP BY chunk_type ORDER BY count DESC').all();
  return { ...base, staleCount, lowConfCount, valueDist, sourceTypeDist, domainDist, chunkTypeDist };
}
...
module.exports = { SCHEMA, openDb, getFileMeta, deleteFileChunks, insertChunks, search, getAdjacentChunks, getStats, getDetailedStats, getAllFilePaths, getChunksByFile };
