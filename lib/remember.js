...
const { gateCheck } = require('./quality-gate');
...
function remember(workspace, content, { tag = 'fact', date = null, qualityGate = null } = {}) {
  ...
  // Quality gate: reject junk before writing (only when config provided)
  if (qualityGate) {
    const gate = gateCheck(sanitized, qualityGate);
    if (!gate.pass) {
      return { filePath: null, created: false, line: null, skipped: true, gateRejected: true, reason: gate.reason };
    }
  }
  ...
  // Avoid double-tagging: if content already starts with the same tag, skip prepend
  const alreadyTagged = new RegExp(`^\\[${tag}\\]`, 'i').test(sanitized);
  const line = alreadyTagged ? `- ${sanitized}` : `- [${tag}] ${sanitized}`;
