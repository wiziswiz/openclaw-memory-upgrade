...
const DEFAULTS = {
  ...
  valueScoring: { enabled: true, boostRange: [0.7, 1.3] },
  synonyms: { enabled: true, penalty: 0.85 },
  diversity: {
    contextEnabled: true,
    recallEnabled: false,
    maxPerFile: 3,
  ...
  promote: { enabled: false, minValueScore: 0.72, maxCandidatesPerRun: 20 },
  domainLabels: {},
};
    ...
    for (const d of dirs) {
      if (seg === '**') {
        ...
        const collectDirs = (dir) => {
          ...
          } catch (_) { /* expected: directory may not exist or be unreadable */ }
        };
        collectDirs(d);
      } else if (seg === '*') {
...
function resolveIncludes(workspace, config) {
  ...
  function add(absPath) {
    ...
    } catch (_) { /* expected: file may not exist yet */ }
  }
