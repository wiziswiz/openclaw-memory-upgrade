...
{
  const r = resolveTemporalQuery('when did I start magnesium?', NOW);
  ...
  assert(r.strippedQuery === 'magnesium?', `Expected stripped to "magnesium?", got: "${r.strippedQuery}"`);
}

// ─── Test 6: no temporal language returns nulls ───
console.log('Test 6: no temporal language returns nulls');
{
  const r = resolveTemporalQuery('how is my magnesium protocol?', NOW);
  ...
  assert(r.strippedQuery === 'how is my magnesium protocol?', 'No temporal → query unchanged');
}
...
console.log('Test 17: Non-attribution keeps exclusions');
{
  ...
  insertChunk(db, {
    content: 'magnesium dopamine protocol daily morning supplement nootropic',
    filePath: 'data/gauntlet/transcripts/2026-02-10.md',
    createdAt: now,
  });
  insertChunk(db, {
    content: 'magnesium protocol started daily 5mg morning dose nootropic',
    filePath: 'memory/2026-02-20.md',
    createdAt: now,
  });

  const result = getRelevantContext(db, 'how is my magnesium protocol?', {
    excludeFromRecall: ['data/gauntlet/transcripts/*.md'],
    alwaysExclude: [],
  });
