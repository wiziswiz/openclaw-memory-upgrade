...
console.log('Test 35: isRuleChunk — normal content not flagged');
{
  ...
  const r2 = isRuleChunk({ content: 'Blood pressure improved from 140/90 to 120/80 since starting the new protocol.', heading: 'Health' });
...
console.log('Test 36: Rule penalty — rule chunks demoted for factual queries');
{
  ...
  insertChunk(db, {
    content: 'Ordered ergonomic desk chair online, purchase confirmed, shipping 2-3 weeks.',
    heading: 'Purchases',
    filePath: 'MEMORY.md',
    chunkType: 'fact',
    confidence: 1.0,
  ...
  if (result.chunks.length >= 2) {
    ...
    const factIdx = result.chunks.findIndex(c => c.content.includes('ergonomic'));
