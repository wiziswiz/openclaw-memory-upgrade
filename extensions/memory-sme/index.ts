...
const memoryPlugin = {
  ...
  async register(api: any) {
    ...
    // Pass plugin config options through to SME workspace config
    const smeCreateOptions: Record<string, any> = { workspace };
    if (cfg.halfLifeDays !== undefined) smeCreateOptions.halfLifeDays = cfg.halfLifeDays;
    if (cfg.excludeFromRecall !== undefined) smeCreateOptions.excludeFromRecall = cfg.excludeFromRecall;
    if (cfg.include !== undefined) smeCreateOptions.include = cfg.include;
    if (cfg.minConfidence !== undefined) smeCreateOptions.minConfidence = cfg.minConfidence;
    const engine = sme.create(smeCreateOptions);
